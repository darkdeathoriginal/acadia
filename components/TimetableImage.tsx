      ...
      >
        ...
        <div className="flex items-center justify-between mb-8 pb-4 shrink-0 px-2 border-b border-[#2A2A2F]">
          <div className="flex flex-col">
            <h1 className="text-[52px] font-black text-white tracking-tight leading-tight mb-1">
              Class Schedule
            </h1>
            <p className="text-[22px] text-gray-500 font-medium">
              Acadia • Official Timetable Export
            </p>
          </div>

          <div className="flex gap-6">
            <div className="flex flex-col items-end justify-center px-10 py-5 bg-[#1C1C21] rounded-2xl border border-[#2A2A2F] shadow-sm">
              <span className="text-[14px] text-gray-500 font-bold mb-1 tracking-widest uppercase">
                SECTION
              </span>
        ...
        <div className="flex-1 flex flex-col gap-[14px] min-h-0 pl-2">
          {/* Times Header */}
          <div
            className="grid gap-[14px] shrink-0"
            style={{
              gridTemplateColumns: `110px repeat(${activeTimes.length}, minmax(0, 1fr))`,
            }}
          >
            ...
            {activeTimes.map((t, i) => (
              <div
                key={i}
                className={`flex flex-col items-start justify-center p-3 px-5 rounded-2xl ${bgCard} border ${borderCard}`}
              >
                <span className="text-white font-bold text-[18px] leading-tight mb-1 tracking-wide">
                  {t.split(" - ")[0]}
                </span>
                <span className="text-gray-500 font-semibold text-[13px] leading-tight">
                  {t.split(" - ")[1]}
                </span>
              </div>
            ))}
          ...
          {[1, 2, 3, 4, 5].map((dayIdx) => {
            const dayData =
              timetable[String(dayIdx - 1)] || timetable[String(dayIdx)] || {};

            return (
              <div
                key={dayIdx}
                className="grid gap-[14px] min-h-0 flex-1"
                style={{
                  gridTemplateColumns: `110px repeat(${activeTimes.length}, minmax(0, 1fr))`,
                }}
              >
                ...
                <div
                  ...
                  style={{ width: `110px` }}
                >
                  <span className="text-gray-500 font-bold text-[15px] tracking-[0.2em] uppercase mb-1">
                    DAY
                  </span>
                  <span className="text-white font-black text-[42px] leading-none">
                    {dayIdx}
                  </span>
                </div>
                ...
                {activeTimes.map((t, i) => {
                  ...
                  const isEmpty =
                    !rawTitle ||
                    rawTitle.toLowerCase() === "na" ||
                    rawTitle.toLowerCase() === "n/a" ||
                    rawTitle.toLowerCase() === "no class" ||
                    rawTitle === "-";
                  ...
                  if (isEmpty) {
                    return (
                      <div
                        key={i}
                        className="rounded-2xl border border-[#2A2A2F] border-dashed flex items-center justify-center bg-transparent min-w-0 min-h-0"
                      >
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col min-w-0">
                        <span
                          className={`text-[19px] font-bold leading-[1.2] mb-[6px] line-clamp-3 break-words ${titleColor}`}
                        >
                          {displayTitle}
                        </span>
                        <span className="text-[13px] font-semibold text-gray-400 opacity-80 truncate">
                          {slot?.code || ""}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between items-end mt-1 min-w-0">
                        <span
                          className={`px-[10px] py-[3px] shrink-0 rounded-md text-[11px] font-bold uppercase tracking-widest ${bgTag} ${tagColor}`}
                        >
                          {isLab ? "LAB" : "THRY"}
                        </span>
                        <span className="text-[13px] text-gray-400 font-bold truncate ml-2">
                          {slot?.room ? slot.room.split("-")[0] : "NA"}
                        </span>
                      </div>
                    </div>
                  );
